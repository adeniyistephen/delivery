	...
	defer func() {
		if mustRollback {
			...
			if err := tx.Rollback(); err != nil {
				log.Errorw("unable to rollback tran", "traceid", "ERROR", err)
			}
		}
	}()
...
// logging and tracing.
func NamedExecContext(db sqlx.ExtContext, query string, data interface{}) error {
	q := queryString(query, data)
	log.Println(q)

	if _, err := sqlx.NamedExecContext(context.Background(), db, query, data); err != nil {
		return err
	}
...
func NamedQueryStruct(db sqlx.ExtContext, query string, data interface{}, dest interface{}) error {
	q := queryString(query, data)
	log.Println(q)
	rows, err := sqlx.NamedQueryContext(context.Background(), db, query, data)
	if err != nil {
		return err
	}
	defer rows.Close()
